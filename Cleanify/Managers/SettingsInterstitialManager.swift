...
class SettingsInterstitialManager: NSObject, FullScreenContentDelegate {
    static let shared = SettingsInterstitialManager()
    ...
    func preloadInterstitialAd() {
        ...
        remoteConfig.fetchAndActivate { [weak self] status, error in
            ...
            let adUnitID = remoteConfig.configValue(forKey: "setting_inter_id").stringValue ?? "ca-app-pub-3940256099942544/4411468910"
            let flagStr = remoteConfig.configValue(forKey: "setting_inter_flag").stringValue ?? ""
            let flag = Int(flagStr) ?? 3
            
            print("[SettingsInterstitialManager] Remote Config: ID=\(adUnitID), Flag=\(flag)")
            
            if self.shouldShowAd(flag: flag, key: "setting_inter") && (adUnitID.contains("ca-app-pub-") || adUnitID.contains("/")) {
                InterstitialAd.load(with: adUnitID, request: Request()) { [weak self] ad, error in
                    self?.isLoading = false
                    if let error = error {
                        print("[SettingsInterstitialManager] Failed to load: \(error.localizedDescription)")
                        return
                    }
                    self?.interstitialAd = ad
                    self?.interstitialAd?.fullScreenContentDelegate = self
                    print("[SettingsInterstitialManager] Preloaded successfully!")
                }
            } else {
                self.isLoading = false
            }
    ...
    func showAdOnBack(from vc: UIViewController, dismissAction: @escaping () -> Void) {
        ...
        guard shouldShowAd(flag: flag, key: "setting_inter") else {
            dismissAction()
            return
        }
        ...
        if let lastShow = lastShowTime, now.timeIntervalSince(lastShow) < 10.0 {
            print("[SettingsInterstitialManager] Cooldown active (<10s). Skipping ad.")
            dismissAction()
            return
        }
        ...
        ad.present(from: vc)
        interstitialAd = nil
        markAdShown(flag: flag, key: "setting_inter")
        preloadInterstitialAd()
    }
    ...
    func adDidDismissFullScreenContent(_ ad: FullScreenPresentingAd) {
        print("[SettingsInterstitialManager] Ad dismissed by user.")
    ...
    func ad(_ ad: FullScreenPresentingAd, didFailToPresentFullScreenContentWithError error: Error) {
        print("[SettingsInterstitialManager] Failed to present ad: \(error.localizedDescription)")
        let action = pendingDismissAction
        pendingDismissAction = nil
        action?()
    }
